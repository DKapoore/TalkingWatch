...
const Clock = (() => {
  ...
  function updateRing() {
    ...
    const fwRing = document.getElementById("floating-widget-progress");
    if (fwRing) {
      const FW_CIRC = 2 * Math.PI * 28;
      fwRing.style.strokeDasharray = `${FW_CIRC}`;
      fwRing.style.strokeDashoffset = `${FW_CIRC * (1 - fraction)}`;
    }
    const bar = document.getElementById("linear-progress");
    if (bar) bar.style.width = `${Math.round(fraction * 100)}%`;
  }

  function fmtAmPm(d) { return d.getHours() < 12 ? "AM" : "PM"; }
  ...
  function updateDisplay() {
    ...
    $("#clock-display").textContent = timeStr;
    if ($("#clock-ampm")) $("#clock-ampm").textContent = fmtAmPm(now);
    if ($("#floating-widget-time")) $("#floating-widget-time").textContent = timeStr;

    if (nextAnnounceAt) {
      $("#next-time").textContent = `${pad2(nextAnnounceAt.getHours())}:${pad2(nextAnnounceAt.getMinutes())}`;
      if ($("#next-ampm")) $("#next-ampm").textContent = fmtAmPm(nextAnnounceAt);
      if ($("#next-countdown")) $("#next-countdown").textContent = fmtCountdown(nextAnnounceAt);
    } else {
      if ($("#next-ampm")) $("#next-ampm").textContent = "";
      if ($("#next-countdown")) $("#next-countdown").textContent = "—:—";
    }
    updateRing();
  }
  ...
  function tick() {
    ...
    if (now >= nextAnnounceAt) {
      ...
      const la = $("#last-announced");
      if (la) la.textContent = `Last announced ${pad2(target.getHours() % 12 === 0 ? 12 : target.getHours() % 12)}:${pad2(target.getMinutes())} ${fmtAmPm(target)}. Volume is controlled by your phone, Bluetooth, or car audio.`;
    }
  }
...
function buildIntervalChips() {
  const row = $("#interval-chips");
  row.innerHTML = "";
  INTERVAL_OPTIONS.forEach((n) => {
    const chip = document.createElement("button");
    chip.type = "button";
    chip.className = "chip";
    chip.dataset.value = String(n);
    chip.textContent = `${n} min`;
    chip.addEventListener("click", () => setInterval_(n));
    row.appendChild(chip);
  });
  const customChip = document.createElement("button");
  customChip.type = "button";
  customChip.className = "chip";
  customChip.dataset.value = "custom";
  customChip.textContent = "Custom";
  customChip.addEventListener("click", () => {
    $("#btn-settings").click();
    $("#set-interval").value = "custom";
    $("#set-interval").dispatchEvent(new Event("change"));
  });
  row.appendChild(customChip);
}

function setInterval_(minutes) {
  prefs.interval = minutes;
  savePrefs(prefs);
  applyPrefsToForm();
  if (Clock.isRunning()) { Clock.stop(); Clock.start(); KeepAlive.start(); }
}
...
function applyPrefsToForm() {
  ...
  const drivingBtn = $("#toggle-driving-mode-pill");
  if (drivingBtn) {
    drivingBtn.classList.toggle("active", prefs.drivingMode);
    drivingBtn.setAttribute("aria-pressed", String(prefs.drivingMode));
  }
...
function updateDateDisplay() {
  ...
  if (!el) return;
  const now = new Date();
  el.textContent = now.toLocaleDateString(undefined, { weekday: "long", day: "numeric", month: "short" });
}
setInterval(updateDateDisplay, 60000);

function updateMetaRow() {
  const langLabel = LANGUAGES.find((l) => l.code === prefs.language).label;
  const voiceLabel = pickVoice(prefs.language, prefs.voiceURI);
  const interval = prefs.interval === "custom" ? `${prefs.customInterval} min` : `${prefs.interval} min`;
  $("#meta-interval").textContent = `Every ${interval}`;
  $("#meta-language").textContent = voiceLabel ? `${langLabel} · ${voiceLabel.name.split(" ")[0]}` : langLabel;

  $all("#interval-chips .chip").forEach((chip) => {
    const isCustom = chip.dataset.value === "custom";
    const active = isCustom ? prefs.interval === "custom" : String(prefs.interval) === chip.dataset.value;
...
function wireMain() {
  ...
  $("#toggle-driving-mode-pill").addEventListener("click", () => {
    prefs.drivingMode = !prefs.drivingMode;
    document.body.dataset.driving = prefs.drivingMode ? "true" : "false";
    $("#toggle-driving-mode-pill").classList.toggle("active", prefs.drivingMode);
    $("#toggle-driving-mode-pill").setAttribute("aria-pressed", String(prefs.drivingMode));
    savePrefs(prefs);
  });

  $("#btn-minimize").addEventListener("click", () => setMinimized(true));
  $("#floating-widget").addEventListener("click", () => setMinimized(false));
...
/* Minimized = an in-app floating button state. The clock/speech engine is
   untouched by this — it keeps running against the same DOM nodes, which
   still exist (just visually collapsed via CSS), so announcements continue
   exactly the same whether minimized or full-screen. */
function setMinimized(value) {
  document.body.dataset.minimized = value ? "true" : "false";
}
